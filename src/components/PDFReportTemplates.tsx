...
import { SavedMatch, Role, ActionType, GoalieAction, Period } from '../types/futsal';

// ── PITCH SYSTEMS (copied from MatchTracker) ─────────────────────
const GameState = {
  FOUR_VS_FOUR: "FOUR_VS_FOUR",
  PJ_ATTACK: "PJ_ATTACK",
  PJ_DEFENSE: "PJ_DEFENSE",
  INFERIORITY: "INFERIORITY",
  THREE_VS_THREE: "THREE_VS_THREE",
  SUPERIORITY: "SUPERIORITY",
};

const PITCH_SYSTEMS: Record<string, { left: number; top: number; label: string }[]> = {
...
const formatPlayerTime = (totalSeconds: number): string => {
...
// ── TYPES ────────────────────────────────────────────────────────
type Player = SavedMatch['players'][0];

export interface PDFReportRef {
  exportTeamPDF: () => Promise<void>;
  exportGoalkeeperPDF: () => Promise<void>;
}
...
// ── HELPER: capture div as PDF page ─────────────────────────────
...
// ── COMPONENT ────────────────────────────────────────────────────
const PDFReportTemplates = forwardRef<PDFReportRef, Props>(({ match, goals, opponentGoals }, ref) => {
  ...
  useImperativeHandle(ref, () => ({
    exportTeamPDF: async () => {
      await captureToPDF(
        [pdfPage1Ref, pdfPage2Ref, pdfPage3Ref, pdfPage4Ref, pdfPage5Ref, pdfPage6Ref],
        `global_jugadores_${(match.teamName || 'equipo').replace(/\s+/g, '_')}_${Date.now()}.pdf`
      );
    },
    exportGoalkeeperPDF: async () => {
      await captureToPDF(
        [pdfGkPage1Ref, pdfGkPage2Ref, pdfGkPage3Ref],
        `porteros_${(match.teamName || 'equipo').replace(/\s+/g, '_')}_${Date.now()}.pdf`
      );
    },
  }));

  // Derive local/rival players  
  const localPlayers = match.players?.filter((p: Player) => !p.isOpponent && p.role !== Role.COACH && p.role !== Role.DELEGATE) ?? [];
  const rivalPlayers = match.players?.filter((p: Player) => p.isOpponent && p.role !== Role.COACH && p.role !== Role.DELEGATE) ?? [];
...
{(() => {
        ...
        const allTeamsForPDF = [
          { players: match.players.filter(p => !p.isOpponent && p.role !== Role.COACH && p.role !== Role.DELEGATE), name: match.teamName, accent: '#3b82f6' },
          { players: match.players.filter(p => p.isOpponent && p.role !== Role.COACH && p.role !== Role.DELEGATE), name: match.opponentName, accent: '#ef4444' },
        ...
        {(() => {
                ...
                return (
                  <>
                    <Header page={1} total={allTeamsForPDF.length * 3} mainTeam={match.teamName} vsTeam={match.opponentName} accent="#3b82f6" />
        ...
        {(() => {
                ...
                return (
                  <>
                    <Header page={4} total={total} mainTeam={match.opponentName} vsTeam={match.teamName} accent="#ef4444" />
                    <div style={{ marginBottom: 8, ...sectionLabelStyle }}>1. estadísticas por posición — {team.name.toLowerCase()}</div>
                    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 10 }}>
                      <thead>
                        <tr style={{ borderBottom: '1px solid #e2e8f0' }}>
...
{(() => {
        const allGkPlayers = match.players.filter(p => p.role === Role.GOALKEEPER && !p.isOpponent);
        const allGkRivalPlayers = match.players.filter(p => p.role === Role.GOALKEEPER && p.isOpponent);
        const allGkTeams = [
          { players: allGkPlayers, name: match.teamName, vsName: match.opponentName, accent: '#3b82f6', score: `${goals} — ${opponentGoals}` },
          { players: allGkRivalPlayers, name: match.opponentName, vsName: match.teamName, accent: '#ef4444', score: `${opponentGoals} — ${goals}` },
        ].filter(t => t.players.length > 0);
            ...
            {allGkTeams.map((team, ti) => (
              <React.Fragment key={ti}>
                ...
                <div ref={ti === 0 ? pdfGkPage3Ref : undefined} style={gkPageStyle}>
                  ...
                  {team.players.map(p => {
                    const ev1 = match.events.filter(e => e.playerIds.includes(p.id) && isFirstHalf(e));
                    const ev2 = match.events.filter(e => e.playerIds.includes(p.id) && isSecondHalf(e));
                  ...
                  {isEditingTeams ? (
                    <div className="space-y-3">
                      <input
                        ...
                        onBlur={(e) =>
                          updateTeams(e.target.value, match.opponentName)
                        }
                      />
                      <input
                        className="w-full bg-black/40 border border-white/10 rounded px-2 py-1 text-xs outline-none focus:border-blue-500"
                        ...
                        onBlur={(e) =>
                          updateTeams(match.teamName, e.target.value)
                        }
                      />
                    </div>
                  ) : (
          ...
          <div className="flex-1 p-4 custom-scrollbar overflow-y-auto flex flex-col h-full">
            <AnimatePresence mode="wait">
                  ...
                  <div className="flex flex-col gap-4">
                    <div className="flex items-center justify-between border-b border-white/5 pb-2">
                      <h4 className="text-[10px] font-black text-red-500 uppercase tracking-widest flex items-center gap-2">
                        <Shield size={14} /> Porteros Rivales ({match.opponentName || "Rival"})
                      </h4>
                    </div>
              ...
              {activeTab === "history" && (
                ...
                >
            {(() => {
                    const eventsByPeriod = match.events.reduce((acc, event) => {
                  ...
                  <div ref={dynamicExportRef} className={`flex flex-col gap-6 py-2 px-1 bg-[#0A0B0E] ${isExporting ? "p-6" : ""}`}>
                    {isExporting && (
                      <div className="flex flex-col items-center justify-center pb-4 border-b border-white/10 mb-2">
                        <h2 className="text-2xl font-black text-white uppercase tracking-widest text-center mb-1">
                          {showRivalStats ? match.opponentName : match.teamName}
                        </h2>
                        <p className="text-sm font-black text-blue-400 uppercase tracking-widest text-center">
                          {statsView === "team" ? "Estadísticas del Equipo" :
                           ...
                           statsView === "maps" ? `Mapas - ${
                             ...
                             "Pérdidas"
                           }${selectedMapPlayerId !== "all" ? ` - Jugador: ${match.players.find(p => p.id === selectedMapPlayerId)?.name}` : " - Equipo Completo"}`
                           : "Estadísticas"}
                        </p>
                      </div>
                    )}
                      ...
                      <div className={`${showRivalStats ? "bg-red-600/10 border-red-500/20 text-red-400" : "bg-blue-600/10 border-blue-500/20 text-blue-400"} border p-4 rounded-3xl shadow-xl mb-4`}>
                        ...
                        <div className="space-y-3 mb-6 bg-black/20 p-4 rounded-2xl border border-white/5">
                          {[
                            ...
                            { 
                              label: "Tiros", 
                              local: match.events.filter(e => (e.type === ActionType.SHOT || e.type === ActionType.GOAL || e.type === GoalieAction.GOAL_CONCEDED) && !e.metadata?.isOpponent).length,
                              rival: match.events.filter(e => (e.type === ActionType.SHOT || e.type === ActionType.GOAL || e.type === GoalieAction.GOAL_CONCEDED) && e.metadata?.isOpponent).length,
                              color: "bg-blue-500" 
                            },
                            { 
                              label: "Faltas", 
                            ...
                            return (
                              <div key={item.label} className="space-y-1">
                                <div className="flex justify-between text-[7px] font-black uppercase tracking-tighter text-slate-500">
                                  <span>{item.local} {match.teamName}</span>
                                  <span className="text-slate-300">{item.label}</span>
                                  <span>{match.opponentName} {item.rival}</span>
                                </div>
                        ...
                        <div className="flex flex-col gap-1 border-b border-white/5 pb-2">
                          <div className={`${showRivalStats ? "bg-red-600/10 border-red-500/20" : "bg-blue-600/10 border-blue-500/20"} rounded-xl p-3 flex items-center justify-between`}>
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              {(() => {
                                      const totalSaves =
                                        match.players.filter(p => p.isOpponent === showRivalStats).reduce(
                      ...
                      <div className={`${showRivalStats ? "bg-red-600/10 border-red-500/20" : "bg-blue-600/10 border-blue-500/20"} border p-4 rounded-3xl`}>
                        <div className="flex items-center justify-between mb-6">
                          <h4 className="text-[10px] font-black text-white uppercase tracking-widest flex items-center gap-2">
                            ...
                            Configuración {showRivalStats ? match.opponentName || "Equipo Rival" : match.teamName || "Mi Equipo"}
                          </h4>
                          <button
                        ...
                        <div className="flex gap-4 mb-6">
                          <div className="flex-1">
                            ...
                            <input
                              type="text"
                              value={showRivalStats ? match.opponentName : match.teamName}
                              onChange={(e) =>
                                setMatchData((prev) => ({
                                  ...prev,
          ...
          <div className="bg-white/5 border border-white/5 rounded-2xl flex flex-col backdrop-blur-md relative overflow-hidden flex-1 min-h-0">
            ...
            <div className="p-1 border-b border-white/5 flex items-center justify-between relative bg-black/20 flex-shrink-0">
              <h4 className={`text-[8px] font-black uppercase tracking-widest flex items-center gap-2 ${pitchView === 'local' ? 'text-blue-400' : 'text-red-400'}`}>
                <MapIcon size={8} /> {pitchView === 'local' ? (match.teamName || "NUESTRO") : (match.opponentName || "RIVAL")} - {pitchView === 'local' ? gameState : rivalGameState}
              </h4>
            ...
            {(() => {
                    ...
                    const onPitchPlayers = match.players.filter((p) => p.isOnPitch && !!p.isOpponent === isOpp);
                        ...
                        {currentSlots.map((slot, index) => {
                          ...
                          return (
                            <div
                              ...
                              onClick={() => {
                                if (swapSelection) {
                                  const p = match.players.find(pl => pl.id === swapSelection);
                          ...
                          return (
                            ...
                            >
                              <PlayerCard
                                ...
                                onClick={() => {
                                  if (swapSelection) {
                                    const swapper = match.players.find(p => p.id === swapSelection);
                ...
                <div className="min-h-[200px] flex flex-col items-center justify-center">
                  {pendingAction.step === "player" && (
                    <div className="w-full space-y-4">
                      ...
                      <PlayerSelector
                        players={match.players.filter(p => 
          ...
          <>
            ...
            >
              <h3 className="text-sm font-black uppercase mb-4 text-blue-400">
                {match.players.find((p) => p.id === editingPlayerId)
                  ?.role === Role.COACH ||
                match.players.find((p) => p.id === editingPlayerId)
                  ?.role === Role.DELEGATE
                  ? "Editar Técnico"
      ...
      <AnimatePresence>
        {activeSlotToAdd && (
          <BenchRadialMenu
            ...
            benchPlayers={match.players.filter((p) => {
      ...
      <AnimatePresence>
        {swapSelection && (
          <BenchRadialMenu
            isOpponent={match.players.find(pl => pl.id === swapSelection)?.isOpponent}
            benchPlayers={match.players.filter((p) => {
              const swapper = match.players.find(
      ...
      <AnimatePresence>
        {activeActionPlayerId && (
          <PlayerActionRadialMenu
            player={
              match.players.find((p) => p.id === activeActionPlayerId)!
            }
...
PDFReportTemplates.displayName = 'PDFReportTemplates';
export default PDFReportTemplates;
